...
import { faCloudUpload, faTrash, faSave } from "@fortawesome/free-solid-svg-icons";
...
const AddImagesSolarBattery: React.FC<AddImagesSolarBatteryProps> = ({ onImagesChange, disabled = false }) => {
  ...
  const imageCategories = [
    {
      id: 'front_house_media_id',
      title: 'ภาพหน้าบ้านลูกค้า'
    },
    {
      id: 'electricity_bill_media_id',
      title: 'ภาพมิเตอร์หรือบิลค่าไฟ'
    },
    {
      id: 'consumer_unit_media_id',
      title: 'ภาพตู้ Consumer Unit หรือ MDB'
    },
    {
      id: 'roof_media_id',
      title: 'ภาพหลังคาบ้าน'
    ...
    {
      id: 'inverter_location_media_id',
      title: 'ภาพจุดติดตั้ง Inventer'
    },
    {
      id: 'battery_location_media_id',
            ...
            return (
              <div key={category.id} className="bg-white border border-[#E1D2FF] rounded-lg p-2 mb-4">
                ...
                {image ? (
                  ...
                  <div className="text-center border border-[#E1D2FF] rounded-lg p-2">
                    <input
            ...
            return (
              <div key={category.id} className="bg-white border border-[#E1D2FF] rounded-lg p-4">
                ...
                {image ? (
                  ...
                  /* Upload Section */
                  <div className="flex flex-row text-center py-8 px-3 gap-4 items-center justify-center border border-[#E1D2FF] rounded-lg">
                     <input
                      id={`upload-${category.id}`}
                      type="file"
                      accept=".png,.jpg,.jpeg"
                    ...
                    <button
                      type="button"
                      onClick={() => document.getElementById(`upload-${category.id}`)?.click()}
                      className="pea-button-outline my-2 !p-2 text-nowrap rounded-full h-fit"
                      disabled={uploading || disabled}
                    >
                      <FontAwesomeIcon icon={faCloudUpload} className="mr-2" />
                      {uploading ? 'กำลังอัพโหลด...' : 'อัพโหลดรูปภาพ'}
